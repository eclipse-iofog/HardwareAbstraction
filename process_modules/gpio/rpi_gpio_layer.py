...
class GPIORPiRESTRequestProcessModule(RESTRequestProcessModule):
    ...
    def process_get_request(self, http_handler):
        response = None
        if HAL_GPIO_RPI_CLEANUP_URL in http_handler.path:
            response = self._cleanup_gpio()
        elif HAL_GPIO_RPI_SET_BCM_MODE_URL in http_handler.path:
            response = self._set_mode()
        elif HAL_GPIO_RPI_SET_BOARD_MODE_URL in http_handler.path:
            response = self._set_mode(board=True)
        else:
            raise HALException(message='This url is not supported or has invalid HTTP Method: {}'.format(http_handler.path))
    ...
    def process_post_request(self, http_handler, data):
        ...
        elif HAL_GPIO_RPI_SET_HIGH_PIN_URL in http_handler.path:
        ...
        try:
            ...
            if current_mode is None:
                if board:
                    ...
                    return 'CPIO mode set successfully'
            else:
    ...
    def _set_up_pins(data):
        for pin in data:
            if HAL_GPIO_RPI_SET_UP_PIN_NUMBER_PROPERTY_NAME in pin and HAL_GPIO_RPI_SET_UP_PIN_TYPE_PROPERTY_NAME in pin:
                ...
                HALException(message='{} and {} are required to set up pin.'.format(
    ...
    @staticmethod
    def _set_pin_value(data):
        values = {}
        for pin in data:
            try:
                value = GPIO.HIGH
                if pin[HAL_GPIO_RPI_SET_UP_PIN_VALUE_PROPERTY_NAME] == 'low':
                    value = GPIO.LOW
                if pin[HAL_GPIO_RPI_SET_UP_PIN_NUMBER_PROPERTY_NAME]:
                    GPIO.output(pin, value)
                    values[pin] = 'ok'
                else:
                    values[pin] = '{} property is required'.format(HAL_GPIO_RPI_SET_UP_PIN_NUMBER_PROPERTY_NAME)
            except Exception as e:
                values[pin] = str(e)
        return values

    @staticmethod
    def _set_pin_value_high(data):
        values = {}
        for pin in data:
            try:
